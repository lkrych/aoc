...
int parentBags(std::map<std::string, std::vector<std::string>> adjacency_list, std::string target, std::map<std::string, bool> *parent_check) {
    int count = 0;
    std::vector<std::string> parents = adjacency_list[target];
...
int baggageClaim(std::vector<std::string> arr, std::string target) {
    ...
    for(int i = 0; i < arr.size(); i++) {
        row = arr[i];
        std::map<std::string, std::vector<std::string>> entry = parseBags(row);
